...
Page({
    ...
    nextstep(e) {
        ...
        if (steps == 1) {
            if (this.data.channelType == 4 && this.data.settlementLogo == '对私' && this.data.merchantType == 2) {
                wx.showToast({
                    icon: 'none',
                    title: '结算标识选择错误，请重新选择',
                })
                return
            }
            console.log('21321')
            ...
            if (this.verify(this.data.step1)) {
                ...
                if (shopInput.rate1 != undefined) {
                    ...
                    } else if (this.data.channelType == 0 || this.data.channelType == 1) {
                        shopInput.rate = '0'
                    } else {
                        wx.showToast({
                            title: "进件费率错误",
                    ...
                    wx.request({
                        ...
                        success: function(res) {
                            ...
                            if (data3.rateType == 'D1') {
                                ...
                                data3.channel = paymentN
                            } else if (data3.rateType == 'T1') {

                                var paymentN;
                                var thatN
                                for (let i = 0; i < that.data.t1c.length; i++) {
                                    if (that.data.t1c[i] == data3.paymentChannel) {
                                        paymentN = that.data.t1n[i]
                                        thatN = i
                                        break
                                    }
                                }
                                console.log(thatN)
                                that.setData({
                                    rateList: that.data.t1rate[thatN],
                                    rateList1: that.data.t1rateNum[thatN],
                                })
                                data3.channel = paymentN
                            }
